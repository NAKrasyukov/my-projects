...
BEGIN
    ...
    IF (IS_SRVROLEMEMBER(''sysadmin'', N''TECHNODOM\SQL-1CAdmin'') = 0) -- Only add if not sysadmin at server level
    BEGIN
...
BEGIN
    ...
    IF (IS_SRVROLEMEMBER(''sysadmin'', N''TECHNODOM\1c-service'') = 0) -- Only add if not sysadmin at server level
    BEGIN
        IF (IS_MEMBER(''db_owner'') <> 1)
        BEGIN
            -- Add the user to the db_owner role
...
BEGIN
    ...
    IF (IS_SRVROLEMEMBER(''sysadmin'', N''TECHNODOM\SQL-DEV-TEAM'') = 0) -- Only add if not sysadmin at server level
    BEGIN
        IF (IS_MEMBER(''db_owner'') <> 1)
        BEGIN
            -- Add the user to the db_owner role
